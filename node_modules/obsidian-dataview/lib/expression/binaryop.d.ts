...
import { LiteralRepr, LiteralType, LiteralValue } from "data/value";
...
export declare type LiteralReprAll<T extends LiteralTypeOrAll> = T extends "*" ? LiteralValue : T extends LiteralType ? LiteralRepr<T> : any;
/** An implementation for a binary operator. */
export declare type BinaryOpImpl<A extends LiteralValue, B extends LiteralValue> = (first: A, second: B, ctx: Context) => LiteralValue;
/** An implementation of a comparator (returning a number) which then automatically defines all of the comparison operators. */
export declare type CompareImpl<T extends LiteralValue> = (first: T, second: T, ctx: Context) => number;
...
export declare class BinaryOpHandler {
    ...
    evaluate(op: BinaryOp, left: LiteralValue, right: LiteralValue, ctx: Context): Result<LiteralValue, string>;
